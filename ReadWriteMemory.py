...
class Process(object):
    ...
    def get_pointer(self, lp_base_address: hex, offsets: List[hex] = ()) -> int:
        ...
        """
        temp_address = self.readInt16(lp_base_address)
        pointer = 0x0
        if not offsets:
            ...
            for offset in offsets:
                ...
                temp_address = self.readInt16(pointer)
            return pointer
    ...
    def readInt16(self, lp_base_address: int) -> Any:
        """
        ...
        """
        try:
            read_buffer = ctypes.c_int16()
            lp_buffer = ctypes.byref(read_buffer)
            n_size = ctypes.sizeof(read_buffer)
            lp_number_of_bytes_read = ctypes.c_int16(0)
    ...
    def readInt(self, lp_base_address: int) -> Any:

        try:
            read_buffer = ctypes.c_int()
            lp_buffer = ctypes.byref(read_buffer)
            n_size = ctypes.sizeof(read_buffer)
            lp_number_of_bytes_read = ctypes.c_int(0)
            ctypes.windll.kernel32.ReadProcessMemory(self.handle, ctypes.c_void_p(lp_base_address), lp_buffer,
                                                     n_size, lp_number_of_bytes_read)
            return read_buffer.value
        except (BufferError, ValueError, TypeError) as error:
